...
def DataTransform(sample, scaling_rate):
    # weak_aug = scaling(sample, scaling_rate)
    # weak_aug = torch.from_numpy(weak_aug)

    weak_aug = jitter(sample, scaling_rate)

    # weak_aug = sample
    # weak_aug = jitter(permutation(sample.cpu(), max_segments=5, seg_mode="random"))
...
def jitter(x, sigma=0.8):
    return x.cpu() + np.random.normal(loc=0., scale=sigma, size=x.shape)
...
def scaling(x, sigma=1.1):
    ...
    for i in range(x.shape[1]):
        xi = x[:, i, :]
        ai.append(np.multiply(xi.cpu(), factor[:, :])[:, np.newaxis, :])
...
def permutation(x, max_segments=5, seg_mode="random"):
    ...
    for i, pat in enumerate(x):
        if num_segs[i] > 1 and num_segs[i]-1 < x.shape[2]-2:
            if seg_mode == "random":
                # print(f"x.shape[2] - 2:{x.shape[2] - 2}, num_segs[i] - 1:{num_segs[i] - 1}")
